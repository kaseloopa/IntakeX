...
let preloadedFoods=[]; // foods.json will be loaded here
...
function showTab(tab){
  ...
  document.getElementById("vaultTab").style.display=tab==="vault"?"block":"none";
...
function getSuggestions(query){
  ...
  // Combine local vault + preloaded foods
  const combined=[...vault, ...preloadedFoods];
  const matches=combined.filter(f=>f.name.toLowerCase().includes(lowerQuery)).slice(0,10);
...
function addFood(){
  ...
  // Check vault first, then preloaded
  ...
  let caloriesPer100=foodSource?foodSource.calories:0;
  let proteinPer100=foodSource?foodSource.protein:0;

  const calories=(caloriesPer100/100)*grams;
  const protein=(proteinPer100/100)*grams;
...
function editVaultFood(index){
  ...
  const newCal=parseFloat(prompt("Calories per 100g:",food.calories));
